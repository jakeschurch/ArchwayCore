...
def parseDates(startDate, endDate):
    ...
    try:
        ...
        Handler().sendError(
            builder, 'Start Date is not in `mm/dd/YYYY` format.'
        )
    ...
    try:
        ...
        Handler().sendError(
            builder, 'End Date is not in `mm/dd/YYYY` format.'
        )
...
def run(
    ...
    fiscalYearEnd = dt.datetime(year=today.year - 1, month=12, day=31)
    ...
    if getSectorHoldings:
        ...
        holdingsFile = xlWriter.WriteHoldings(outputLoc)
        # txFile = xlWriter.WriteTxLog(outputLoc)

    if getTxLog:
        allTx = Tx.GetComplianceTx(transactions)

    Handler().sendFinished(builder, holdingsFile, None)
